...
if (grepl('theor_ecol$', getwd())){
    setwd('./Pine_dat_1')
} else if (!grepl('Pine_dat_1$', getwd())) {
    stop('Unknown directory. It should be in one ending in "theor_ecol" or "Pine_dat_1".')
}
...
catch_df %>% 
    ...
    xlab('Year') +
    theme(legend.position = 'none') +
    geom_line(data = flow_df, inherit.aes = FALSE, 
              aes(Year, duration))
...
And model $\mu$ with linear model
