---
title: "Pine barrens data"
author: "Jared Beck, Amanda McCormick, Lucas Nell"
date: "12/5/2016"
output:
    slidy_presentation:
        font_adjustment: 0
...
knitr::opts_chunk$set(echo = FALSE, eval = TRUE, comment = '', prompt = FALSE)
library(tidyverse)
library(lme4)
library(lmerTest)
library(GGally)
library(vegan)
source('Nell_preamble.R')
...
P-values for individual environmental variables:
```{r}
rda_partial_pvals
```
...
P-values for random effects using `anova`:
```{r}
...
Procrustes sum of squares: `r signif(proc$ss, 3)`
